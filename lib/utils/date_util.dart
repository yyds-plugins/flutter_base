...
class DateUtil {
  ...
  static String getTimeAgoString(Duration timeAgo) {
    if (timeAgo.inSeconds < 60) {
      return '刚刚';
    } else if (timeAgo.inMinutes < 60) {
      return '${timeAgo.inMinutes}分钟前';
    } else if (timeAgo.inHours < 24) {
      return '${timeAgo.inHours}小时前';
    } else if (timeAgo.inDays < 7) {
      return '${timeAgo.inDays}天前';
    } else {
      return DateUtil.formatDateMs(timeAgo.inMilliseconds, format: 'yyyy-MM-dd');
    }
  }
  ...
  static bool isTimeToday(DateTime? dateTime) {

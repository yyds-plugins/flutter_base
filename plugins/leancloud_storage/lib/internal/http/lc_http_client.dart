...
class _LCHttpClient {
  ...
  _LCHttpClient(this.appId, this.appKey, this.server, this.sdkVersion, this.apiVersion, LCQueryCache? queryCache) {
    _appRouter = new _LCAppRouter(appId, server);
    BaseOptions options =
        new BaseOptions(headers: {'X-LC-Id': appId, 'User-Agent': 'LeanCloud-Flutter-SDK/$sdkVersion'}, contentType: 'application/json');
  ...
  Future get(String path, {Map<String, dynamic>? headers, Map<String, dynamic>? queryParams, Duration? maxCacheAge, CachePolicy? cachePolicy}) async {
    ...
    if (cachePolicy == CachePolicy.networkElseCache) {
      ...
      options = buildConfigurableCacheOptions(
          options: options, maxAge: maxCacheAge ?? new Duration(days: 7), primaryKey: primaryKey, subKey: subKey, forceRefresh: true);
    }
    try {
  ...
  Future<Map<String, dynamic>> _generateHeaders(Map<String, dynamic>? additionalHeaders) async {

...
class _CacheManagerPanelState extends State<CacheManagerPanel> {
  ...
  Widget build(BuildContext context) {
    return Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
          Text("Cache Manager", style: Theme.of(context).textTheme.titleLarge!.copyWith(color: Theme.of(context).primaryColor)),
          Container(height: 50),
          Text("1. Choose mode:", style: Theme.of(context).textTheme.titleSmall!.copyWith(color: Theme.of(context).primaryColor)),
          DropdownButton<_Mode>(
              value: _mode,
              onChanged: (value) => setState(() => _mode = value),
              items: <_Mode>[_Mode.clearByKey, _Mode.clearByKeyAndSubKey, _Mode.clearAll]
                  .map<DropdownMenuItem<_Mode>>((value) => DropdownMenuItem<_Mode>(value: value, child: Text(getTxtByMode(value))))
                  .toList()),
          Container(height: 20),
          ...
          Container(height: 20),
          Text("${getLabel()}. to clear", style: Theme.of(context).textTheme.titleSmall!.copyWith(color: Theme.of(context).primaryColor)),
          Padding(
              padding: EdgeInsets.all(10),
              child: FloatingActionButton(
                  child: Text("Clear", style: Theme.of(context).textTheme.titleSmall!.copyWith(color: Colors.white)), onPressed: () => _clear()))
        ]));
  }
  ...
  void _clear() {
    ...
    if (_mode == _Mode.clearAll) {
      ...
      DioHelper.getCacheManager().deleteByPrimaryKey(_keyController.text, requestMethod: _requestMethodController.text).then(resultPrinter);
    } else if (_mode == _Mode.clearByKeyAndSubKey) {
      DioHelper.getCacheManager()
          .deleteByPrimaryKeyAndSubKey(_keyController.text, requestMethod: _requestMethodController.text, subKey: _subKeyController.text)
          .then(resultPrinter);
    }
  }
  ...
  List<Widget> getRequestMethodViews(BuildContext context) {
    ...
    return [
      Text("2. RequestMethod:", style: Theme.of(context).textTheme.titleSmall!.copyWith(color: Theme.of(context).primaryColor)),
      TextField(controller: _requestMethodController, style: Theme.of(context).textTheme.bodyLarge),
      Container(height: 20),
    ];
  }
  ...
  List<Widget> getKeyViews(BuildContext context) {
    ...
    return [
      Text("3. Key:", style: Theme.of(context).textTheme.titleSmall!.copyWith(color: Theme.of(context).primaryColor)),
      TextField(controller: _keyController, style: Theme.of(context).textTheme.bodyLarge),
      Container(height: 20),
    ];
  }
  ...
  List<Widget> getSubKeyViews(BuildContext context) {
    ...
    return [
      Text("4. Subkey:", style: Theme.of(context).textTheme.titleSmall!.copyWith(color: Theme.of(context).primaryColor)),
      TextField(controller: _subKeyController, style: Theme.of(context).textTheme.bodyLarge),
      Container(height: 20),
    ];
  }
